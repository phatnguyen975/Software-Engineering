...
// Thu thập default metrics của Node.js process: event loop lag, heap size, GC duration, active handles...
const register = client.register;
client.collectDefaultMetrics({ register });

// Histogram đo HTTP request duration, label theo method + route + status
const httpRequestDuration = new client.Histogram({
  name: "http_request_duration_seconds",
  help: "Duration of HTTP requests in seconds",
  labelNames: ["method", "route", "status_code"],
  // Buckets phù hợp với backend Node.js nhỏ: từ 10ms đến 10s
  buckets: [0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10],
});

// Counter đếm tổng requests, dùng để tính error rate
...
// Middleware đo duration cho mọi request
