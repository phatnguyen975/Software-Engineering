...
const SEED_COUNT = 20; // Enough for 15 VUs
const OUTPUT_FILE = path.join(__dirname, "seed-state.json");

async function main() {
  console.log(`=== Starting Data Seeding (${SEED_COUNT} users & orders for Soak test) ===`);
  ...
  for (let i = 0; i < SEED_COUNT; i++) {
    ...
    const email = `soak_${runTimestamp}_${userIndex}@test.local`;
    const password = "Test1234!";
    const name = `Soak User ${userIndex}`;
