...
import org.chatapp.client.ui.admin.reports.ReviewReportDialog;
...
public class SpamReportPanel extends JPanel {
    ...
    private void initControlPanel() {
        ...
        JComboBox<String> sortBy = new JComboBox<>(new String[] { "Time", "Username" });
        ...
        reviewBtn.setFont(new Font("Segoe UI", Font.BOLD, 12));
        reviewBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        reviewBtn.setForeground(Color.WHITE);
        reviewBtn.setBackground(new Color(0, 130, 180));
        ...
        refreshBtn.addActionListener(e -> fetchReportData(currentPage));
    ...
    private void initReportTable() {
        String[] columns = { "Report ID", "Reporter", "Reported", "Message ID", "Reason", "Time", "Status" };
    ...
    private List<String[]> generateDummyReports() {
    ...
    private void fetchReportData(int page) {
        // TODO: Replace with real DB query with LIMIT + OFFSET
        // Example:
        // SELECT * FROM users ORDER BY username ASC LIMIT PAGE_SIZE OFFSET
        // (page-1)*PAGE_SIZE

        List<String[]> allUsers = generateDummyReports();
        totalReports = allUsers.size();
        totalPages = (int) Math.ceil(totalReports * 1.0 / PAGE_SIZE);

        int start = (page - 1) * PAGE_SIZE;
        int end = Math.min(start + PAGE_SIZE, totalReports);
    ...
    private void reloadPagination() {
        ...
        prevBtn.addActionListener(e -> fetchReportData(currentPage - 1));
        ...
        for (int i = start; i <= end; i++) {
            ...
            int pageNumber = i;
            pageBtn.addActionListener(e -> fetchReportData(pageNumber));
        ...
        nextBtn.addActionListener(e -> fetchReportData(currentPage + 1));
        pageControlPanel.add(nextBtn);

        JPanel totalUserPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 5, 5));
        JLabel totalUserLabel = new JLabel("Total Reports:");
        totalUserLabel.setFont(new Font("Segoe UI", Font.BOLD, 12));
        JLabel totalUserValue = new JLabel(String.valueOf(totalReports));
