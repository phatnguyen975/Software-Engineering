...
- [x] B. The practice of applying cloud characteristics to a solution that uses cloud services and features to meet an organization's technical needs and business use cases.
...
- [x] A. Analyze solutions for business needs and requirements.
...
- [x] C. Democratize advanced technologies.
...
- [x] C. Helps you review the state of your workloads and compares them to the latest AWS architectural best practices.
...
- [x] B. Trading consistency, durability, and space for time and latency to deliver higher performance.
...
- [x] A. It reduces configuration errors from manual configuration.
...
- [x] C. Automating deployment of new resources with identical configurations and replacing old resources with updated ones.
...
- [x] C. Decoupling Web servers with Elastic Load Balancing (ELB).
...
- [x] B. Store them off server, such as on Amazon Simple Storage Service (Amazon S3).
...
- [x] C. The programming language of the user interface.
...
- [x] B. Create a secondary (standby) database server and replicate the data.
...
- [x] A. How do I turn off resources that are not in use?
...
- [x] B. Encrypting data in transit and at rest.
...
- [x] B. Two or more Availability Zones.
- [ ] C. Only Local Zones.
...
- [x] C. It is made up of one or more data centers and is designed for fault isolation.
...
- [x] C. To run latency-sensitive portions of applications closer to end users and resources in a specific geography where no Regions exist today.
...
- [x] B. It typically has tens of thousands of servers.
...
- [x] C. It consists of AWS data centers and servers located close to customers designed to deliver services with the lowest latency possible.
...
- [x] B. AWS data centers between the origin server and the edge location that have a longer cache.
...
- [x] B. Test updates on new resources, and then replace old resources with updated ones.
...
- [x] C. It can be used for user authentication/state storage, as well as for securing the infrastructure and optimizing costs.
...
- [x] B. Compliance requirements or to reduce latency.
